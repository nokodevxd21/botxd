  ...
  execute: async (message, args, client, prefix) => {
    ...
    if (player.queue.length === "0" || !player.queue.length) {
      const embed = new EmbedBuilder()
        .setColor(client.embedColor)
        .setDescription(`Now Playing [${player.queue.current.title}](https://discord.gg/2pkvB82NaS) - \`[${convertTime(queue.current.duration)}]\` - [${player.queue.current.requester}]`);
      ...
      if (player.queue.size < 11) {
        const embed = new EmbedBuilder()
          .setColor(client.embedColor)
          .setDescription(`**Now Playing**\n[${player.queue.current.title}](https://discord.gg/2pkvB82NaS) - \`[${convertTime(queue.current.duration)}]\` - [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`
          )
          ...
          .setAuthor({name:`${message.guild.name} Queue`});
        ...
        collector.on("collect", async (button) => {
          if (button.customId === "queue_cmd_but_1") {
            ...
            const embed3 = new EmbedBuilder()
              .setColor(client.embedColor)
              .setDescription(
                `**Now Playing**\n[${player.queue.current.title}](https://discord.gg/presto) - \`[${convertTime(queue.current.duration)}]\` - [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`)
